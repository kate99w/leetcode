...
/*    There are N gas stations along a circular route, where the amount of gas at station i is gas[i].
    ...
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        for (int i = 0; i < gas.length; i++) {
            ...
            if (firstPositive >= 0) {
                prevPositiveSum += gas[i] - cost[i];
                if (prevPositiveSum < 0) {
                    firstPositive = -1;
                    prevPositiveSum = 0;
                }
            }
            if (firstPositive < 0 && gas[i] - cost[i] >= 0) {
                prevPositiveSum = gas[i] - cost[i];
                firstPositive = i;
            }
        }
    ...
    @Test
    public void test() {
        System.out.println(this.canCompleteCircuit(new int[] {3,1,1}, new int[] {1,2,2}));
    }
}
